...
$$ X(\omega) = \sum\_{n=-\infty}^{n=\infty} \textbf{x}[n] \exp(-j \omega n). $$

Recall that $ \exp(-j \omega n) = \cos(\omega n) - j\sin(\omega n)$. This transform is a periodic function with period $2 \pi$,
...
$$ H(\omega) = \sum\_{n=-\infty}^{n=\infty} h[n] \exp(-j \omega n). $$
...
$$ \exp(j \omega*{0} n) \rightarrow H(\omega*{0}) \exp(j \omega\_{0} n). $$
...
$$ \exp(j \omega*{0} n) \rightarrow |H(\omega*{0})| \exp(j (\omega\_{0} n + \angle H(\omega))). $$
...
- Differentiator: $H(\omega) = 1 - \exp(-j \omega) $
